...
public class EasyPopupFragment extends BaseFragment {
    ...
    public static EasyPopupFragment newInstance() {
        EasyPopupFragment fragment = new EasyPopupFragment();
        return fragment;
    }
    ...
    protected int attachLayoutRes() {
        return R.layout.fragment_easy_popup;
    }
    ...
    private void initCustomPop() {
        customPopup = CustomPopup.create().setContext(mContext).apply();
    ...
    private void initAnyBgDimPop() {
        mAnyBgDimPop = EasyPopup.create()
                .setContentView(mContext, R.layout.layout_any)
...
//         .setDimValue(0.4f)  只设置 改的是窗体的    背景 设置了dimview 改变的是dimview 给dimview 添加遮罩
    ...
    private void initCirclePop() {
        mCirclePop = EasyPopup.create().setContentView(mContext, R.layout.layout_circle_comment)
