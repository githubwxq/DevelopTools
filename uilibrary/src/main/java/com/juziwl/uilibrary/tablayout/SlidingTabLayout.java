...
import com.juziwl.uilibrary.tablayout.utils.UnreadMsgUtils;
...
public class SlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorColor = ta.getColor(R.styleable.SlidingTabLayout_tl_indicator_color,
                Color.parseColor(mIndicatorStyle == STYLE_BLOCK ? "#4B6A87" :
                  "#ff6f26"));
        mIndicatorHeight = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_height,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 4 : (mIndicatorStyle == STYLE_BLOCK ? -1 : 3)));
        mIndicatorWidth = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_width,
                dp2px(mIndicatorStyle == STYLE_TRIANGLE ? 10 : 24));
        mIndicatorCornerRadius = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_corner_radius,
                dp2px(mIndicatorStyle == STYLE_BLOCK ? -1 : 3));
        mIndicatorMarginLeft = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_margin_left,
                dp2px(0));
        ...
        mIndicatorMarginBottom = ta.getDimension(R.styleable.SlidingTabLayout_tl_indicator_margin_bottom,
                dp2px(mIndicatorStyle == STYLE_BLOCK ? 7 : 2));
        ...
        mTextSelectColor = ta.getColor(R.styleable.SlidingTabLayout_tl_textSelectColor,
                ContextCompat.getColor(context, R.color.common_333333));
        mTextUnselectColor = ta.getColor(R.styleable.SlidingTabLayout_tl_textUnselectColor,
                ContextCompat.getColor(context, R.color.common_999999));
                    ...
                    if (mViewPager.getCurrentItem() != position) {
                        if (mSnapOnTabClick) {
                            ...
                            if (clickListener != null) {
                                if (clickListener.onTabClick(position)) {
                                    // true 表示 viewpage还是能滑动的
                                    mViewPager.setCurrentItem(position, false);
                                } else {
//                                    、、false 表示viewpage不跟着滑动
                                }
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mIndicatorStyle == STYLE_TRIANGLE) {
            ...
            if (mIndicatorHeight > 0) {
                if (mIndicatorCornerRadius < 0 || mIndicatorCornerRadius > mIndicatorHeight / TEXT_BOLD_BOTH) {
    ...
    public void setTabWidthInPx(float tabWidthPx) {
    ...
    public void setClickListener(OnTabClickListener clickListener) {
        this.clickListener = clickListener;
    }

    private OnTabClickListener clickListener;

    //而外添加接口回调点击回调 点击后viewpage滑不滑 外面处理
