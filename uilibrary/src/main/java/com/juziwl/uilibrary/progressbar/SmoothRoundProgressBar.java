...
public class SmoothRoundProgressBar extends View {
    ...
    private void init(final Context context, final AttributeSet attrs) {
       ...
       // int layout_height = ta.getDimensionPixelSize(3, ViewGroup.LayoutParams.MATCH_PARENT);
        ...
        //获取xml中设置的自定义属性
            ...
            try {
                typedArray = context.obtainStyledAttributes(attrs, R.styleable.common_MyCircleProgressBar);
                strokeWidth = (int) typedArray.getDimension(R.styleable.common_MyCircleProgressBar_common_cpb_strokeWidth, strokeWidth);
                startColor = typedArray.getColor(R.styleable.common_MyCircleProgressBar_common_cpb_startColor, startColor);
                endColor = typedArray.getColor(R.styleable.common_MyCircleProgressBar_common_cpb_endColor, endColor);
                duration = typedArray.getInteger(R.styleable.common_MyCircleProgressBar_common_cpb_duration,duration);
            } finally {
                if (typedArray != null) {
                    typedArray.recycle();
                }
    ...
    protected void onAttachedToWindow() {
        ...
        //默认状态是随sdk不同而不同，5.0以上会一快一慢，这里用线性插值器限定其匀速转动
