package com.juziwl.uilibrary.edittext.verify;
...
public class VerifyCodeEditText extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mEtWidth = typedArray.getDimensionPixelSize(R.styleable.VerifyCodeEditText_vcet_width, 21);
        mEtDivider = getDrawableAttrRes(getContext(), typedArray, R.styleable.VerifyCodeEditText_vcet_divider);
        mEtTextSize = typedArray.getDimensionPixelSize(R.styleable.VerifyCodeEditText_vcet_text_size, 16);
        mEtTextColor = typedArray.getColor(R.styleable.VerifyCodeEditText_vcet_text_color, Color.BLACK);
        mBackgroundFocus = getDrawableAttrRes(getContext(), typedArray, R.styleable.VerifyCodeEditText_vcet_bg_focus);
        mBackgroundNormal = getDrawableAttrRes(getContext(), typedArray, R.styleable.VerifyCodeEditText_vcet_bg_normal);
        mIsPwd = typedArray.getBoolean(R.styleable.VerifyCodeEditText_vcet_is_pwd, false);
        mPwdRadius = typedArray.getDimensionPixelSize(R.styleable.VerifyCodeEditText_vcet_pwd_radius, 5);
        //释放资源
        typedArray.recycle();

        // 当xml中未配置时 这里进行初始配置默认图片
        if (mEtDivider == null) {
            mEtDivider = getDrawable(getContext(), R.drawable.vcet_shape_divider);
        }
        if (mBackgroundFocus == null) {
            mBackgroundFocus = getDrawable(getContext(), R.drawable.vcet_shape_bg_focus);
        }
        if (mBackgroundNormal == null) {
            mBackgroundNormal = getDrawable(getContext(), R.drawable.vcet_shape_bg_normal);
        }
     ...
     * @return
     */
    public  Drawable getDrawableAttrRes(Context context, TypedArray typedArray, @StyleableRes int index) {
     ...
     * @return
     */
    public  Drawable getDrawable(Context context, @DrawableRes int resId) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
