...
public class VerticalDividerItemDecoration extends FlexibleDividerDecoration {
    ...
    @Override
    protected Rect getDividerBoundOne(int position, RecyclerView parent, View child) {
        return null;
    }
    ...
    protected void setItemOffsets(Rect outRect, int position, int childCount, RecyclerView parent) {
        ...
        if (isLastColum(parent, position, getSpanCount(parent), childCount))// 如果是最后一列，则不需要绘制右边
        {
            outRect.set(0, 0, 0, 0);
            return;
        }
