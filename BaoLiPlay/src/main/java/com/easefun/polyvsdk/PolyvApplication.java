...
//		new LoadConfigTask().execute();
		...
		//使用SDK加密串来配置
//		client.setConfig("Y+Keq2jA2xX64EM3K/5Zhoqf9HvN1SJrLXYrW0C5QdwPRv0VONuhVEz7uq4qDk/lm0GoGoFO74ua6E1fiWCUhaomX69/fdCl1vw72qM7tsH3RQ0IteI3D3MqbRVeUPSJ3eUsHvxiF44wUPgzGl5SaQ==", aeskey, iv, getApplicationContext());
		client.setConfig("tI/34nFKNieYwbuWD9H2CrY5hSWVrw0lvcK3kJy+bg4JOdXSnvvctpatWOXxn6NdfCkyUix3q5WgyWvKG77CZlsq5QI4vrGHDKtp1r3ux3AttgKjVNXC70tgR2e/d0G5Njt/No7Jz+4UfmVnA8vQBQ==", aeskey, iv, getApplicationContext());
		//初始化SDK设置
...
//		PolyvDownloaderManager.setDownloadQueueCount(5);
	...
	private void setDownloadDir() {
		String rootDownloadDirName = "polyvdownload1";
        ...
        Log.e("download",downloadDir.getAbsolutePath());
		...
		PolyvDevMountInfo.getInstance().init(this, new PolyvDevMountInfo.OnLoadCallback() {
			...
			public void callback() {
				...
				if (!TextUtils.isEmpty(externalSDCardPath)) {
					...
					dirPath.append(externalSDCardPath).append(File.separator).append("polyvdownload1");
...
//					return;
				...
				File saveDir = new File(PolyvDevMountInfo.getInstance().getInternalSDCardPath() + File.separator + "polyvdownload1");
				if (!saveDir.exists()) {
					saveDir.mkdirs();//创建下载目录
				}
				...
				//设置下载存储目录
				PolyvSDKClient.getInstance().setDownloadDir(saveDir);
				 Log.e("download",saveDir.getAbsolutePath());
//				subDirList.add(saveDir);
