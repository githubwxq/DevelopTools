...
public class PublishCommentActivity extends BaseActivity {
    ...
    private void publishComment() {
        ...
        ToastUtils.showShort(selectList.size() + "张图片被选中");
        ...
        // 获取上传token
        Flowable<String> tokenFlowable = Api.getInstance()
                .getApiService(KlookApi.class).getQiniuToken()
                .compose(ResponseTransformer.handleResult());


        tokenFlowable.flatMap(new Function<String, Flowable<List<String>>>() {
            @Override
...
//                            emitter.onNext(needUpHash);
        ...
        }).flatMap(new Function<List<String>, Publisher<?>>() {
            @Override
            public Publisher<?> apply(List<String> pics) throws Exception {
