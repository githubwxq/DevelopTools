...
public class RomUtils {
    ...
    public static boolean isMIUIV6OrAbove() {
        return getMIUIVersion() >= 4;
    }

    public static boolean isMIUIV9OrAbove() {
        return getMIUIVersion() >= 7;
    }

    private static int getMIUIVersion() {
        String miuiVersionCodeStr = getSystemProperty("ro.miui.ui.version.code");
        if (!TextUtils.isEmpty(miuiVersionCodeStr) && TextUtils.isDigitsOnly(miuiVersionCodeStr)) {
            return Integer.parseInt(miuiVersionCodeStr);
        }
        return 0;
    }
