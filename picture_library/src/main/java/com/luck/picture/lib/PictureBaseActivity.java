...
public class PictureBaseActivity extends FragmentActivity {
    ...
    protected String cameraPath, outputCameraPath, outputVideoPath;
    protected String originalPath;
    protected PictureDialog dialog;
    protected PictureDialog compressDialog;
    protected List<LocalMedia> selectionMedias;
    protected String[] classNames = null;
    protected long videoDuration, videoSize, maxImageSize;
    protected boolean isJudgeImageSize;
    ...
    private void initConfig() {
        ...
        outputVideoPath = config.outputVideoPath;
        statusFont = AttrsUtils.getTypeValueBoolean(this, R.attr.picture_statusFontColor);
        previewStatusFont = AttrsUtils.getTypeValueBoolean(this, R.attr.picture_preview_statusFontColor);
        ...
        checkNumMode = config.checkNumMode = AttrsUtils.getTypeValueBoolean(this, R.attr.picture_style_checkNumMode);
        ...
        isJudgeImageSize = config.isJudgeImageSize;
    }
    ...
    protected void startCrop(String originalPath) {
        ...
        options.setStatusFont(previewStatusFont);
    ...
    protected void onResult(List<LocalMedia> images) {
        dismissCompressDialog();
        if (camera && selectionMode == PictureConfig.MULTIPLE && selectionMedias != null) {
            images.addAll(0, selectionMedias);
        }
        //最后退出当前页面的时所有图片集合数据放在集合里面共外部使用
        Intent intent = PictureSelector.putIntentResult(images);
        setResult(RESULT_OK, intent);
        if (classNames != null && classNames.length > 0) {
            for (String className : classNames) {
                AppManager.getInstance().killActivity(className);
            }
            Intent data = new Intent(intent);
            data.setAction(PictureConfig.ACTION_SELECT_FINISH);
            LocalBroadcastManager.getInstance(this).sendBroadcast(data);
            if (config != null) {
                config.classNames = null;
            }
        }
        closeActivity();
    }
