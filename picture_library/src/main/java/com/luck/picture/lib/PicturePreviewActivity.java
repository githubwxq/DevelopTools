...
//                overridePendingTransition(0, R.anim.a3);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        preview_complete_textColor = AttrsUtils.getTypeValueColor(this, R.attr.picture_ac_preview_complete_textColor);
                    ...
                    if (PictureMimeType.isPictureType(image.getPictureType()) == TYPE_IMAGE && isChecked) {
                        if (isJudgeImageSize) {
                            if (FileUtil.getFileSize(image.getPath()) > maxImageSize) {
                                Toast.makeText(PicturePreviewActivity.this, String.format(Locale.getDefault(), "图片不允许大于%dM",
...
//            imageView.getLayoutParams().width= DisplayUtils.getScreenWidth(PicturePreviewActivity.this);
...
//                            .into(imageView);
                ...
                imageView.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View v) {
                        //显示的话影长  不显示的话
                        if (click) {
                            ObjectAnimator.ofFloat(select_bar_layout, "translationY", 0, dip2px(45)).setDuration(200).start();
                            ObjectAnimator.ofFloat(rl_title, "translationY", 0, -dip2px(48)).setDuration(200).start();
                            click = false;

                        } else {
                            click = true;
                            ObjectAnimator.ofFloat(select_bar_layout, "translationY", dip2px(45), 0).setDuration(200).start();
                            ObjectAnimator.ofFloat(rl_title, "translationY", -dip2px(48), 0).setDuration(200).start();
            ...
            (container).addView(contentView, ViewPager.LayoutParams.MATCH_PARENT, ViewPager.LayoutParams.MATCH_PARENT);
            return contentView;
        }
    }
...
//            overridePendingTransition(0, R.anim.a3);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK) {
            if (requestCode == UCropMulti.REQUEST_MULTI_CROP) {
